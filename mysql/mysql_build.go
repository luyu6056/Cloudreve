...
func New_mysqlBuild() *Mysql_Build {
	...
	return buildPool.Get().(*Mysql_Build)
}
...
func (this *Mysql_Build) Reset(db *MysqlDB) {
	...
	this.sql.raw.Reset()
	this.err = nil
	this.Auto_preload = false
	this.sql.offset.Reset()
}
...
var buildPool = sync.Pool{New: func() interface{} {
	return &Mysql_Build{buffer: new(MsgBuffer), sql: new(sql_buffer)}
}}
...
func (build *Mysql_Build) Table(tablename string) *Mysql_Table {
	if build.sql.table.Len() == 0 {
		build.sql.table.WriteByte('`')
		build.sql.table.Write(Tablepre)
		tablename = strings.ToLower(tablename)
		if tablename[len(tablename)-1] == 'y' {
			build.sql.table.WriteString(tablename[:len(tablename)-1])
			build.sql.table.WriteString("ies`")
		} else {
			build.sql.table.WriteString(tablename)
			build.sql.table.WriteString("s`")
		}
	}
...
func (this *Mysql_Build) Where(conditions string, arg ...interface{}) *Mysql_Build {
	...
	if this.prepare {
		...
		for i := 0; i < len(arg); i++ {
			this.sql.where_prepare_arg = append(this.sql.where_prepare_arg, arg[i])
		}

	} else {
		for i := 0; i < len(arg); i++ {
			conditions = strings.Replace(conditions, "?", Getvalue(arg[i]), 1)
		}
		this.sql.where.WriteString(conditions)
	}
	this.sql.where.WriteByte(')')
	return this

}
func (this *Mysql_Build) AddWhere(conditions string, arg ...interface{}) *Mysql_Build {
	if this.sql.where.Len() == 0 {
		return this.Where(conditions, arg...)
	}
...
func (this *Mysql_Build) AddWhereOr(conditions string, arg ...interface{}) *Mysql_Build {
	if this.sql.where.Len() == 0 {
		return this.Where(conditions, arg...)
	}
	this.sql.where.WriteString(" or (")
	if this.prepare {
		this.sql.where.WriteString(conditions)
		for i := 0; i < len(arg); i++ {
			this.sql.where_prepare_arg = append(this.sql.where_prepare_arg, arg[i])
...
func (this *Mysql_Build) Offset(offset uint64) *Mysql_Build {
	if offset < 0 {
		this.sql.offset.Reset()
	} else {
		this.sql.offset.Write([]byte{32, 111, 102, 102, 115, 101, 116, 32})
		this.sql.offset.WriteString(strconv.FormatUint(offset, 10))
	}
	return this
}

func (this *Mysql_Build) Limit(limit ...uint64) *Mysql_Build {
	...
	if len(limit) == 1 && limit[0] == 0 {
		...
		switch len(limit) {
		case 1:
			this.sql.limit.WriteString(strconv.FormatUint(limit[0], 10))
		case 2:
			this.sql.limit.WriteString(strconv.FormatUint(limit[0], 10))
			this.sql.limit.WriteString(",")
			this.sql.limit.WriteString(strconv.FormatUint(limit[1], 10))
		}
...
func (this *Mysql_Build) Select(s interface{}) (err error) {
	...
	this.buffer.Write(this.sql.offset.Bytes())
	...
	DEBUG(`sql语句`, this.buffer.String(), this.prepare_arg, e)
...
func (this *Mysql_Build) Scan(value ...interface{}) error {
	defer this.Put()
	if this.err != nil {
		return this.err
	}
	...
	this.buffer.Write(this.sql.lock.Bytes())
	res, err := querySlice(this.buffer.Bytes(), this.prepare_arg, this.db, this.Transaction)
	if err != nil {
		err = errors.New(`执行SelectMap出错,sql错误信息：` + err.Error() + `,错误sql：` + this.buffer.String() + "  参数 " + fmt.Sprintf("%+v", this.prepare_arg))
		return err
	}
	if len(res) > 0 {
		for k, i := range value {
			...
			switch v := i.(type) {
			case *int:
				_i, _ := strconv.Atoi(res[0][k])
				*v = _i
			default:
				panic("scan未设置" + reflect.TypeOf(i).String())
			}
		}
	}
	return nil
...
func (this *Mysql_Build) Count() (res int, err error) {
	...
	this.buffer.Reset()
	this.buffer.Write([]byte{115, 101, 108, 101, 99, 116, 32}) //select count(
	if field := this.sql.field.String(); field == "*" {
		this.sql.field.Reset()
		this.sql.field.WriteString("count(*) as count")
	}
	this.buffer.Write(this.sql.field.Bytes())
	this.buffer.Write([]byte{32, 99, 111, 110, 117, 116, 32, 102, 114, 111, 109, 32}) //) as conut from
	this.buffer.Write(this.sql.table.Bytes())
	this.buffer.Write(this.sql.joinTable.Bytes())
	this.buffer.Write(this.sql.on.Bytes())
	this.buffer.Write(this.sql.where.Bytes())
...
func (this *Mysql_Build) Insert(i interface{}) (id int64, err error) {
	...
	case reflect.Struct:
		...
		if fieldColumnMap == nil {
			return 0, errors.New("未初始化的struct 名称 " + r.Name())
		}
		for i1 := 0; i1 < r.NumField(); i1++ {

			field_t := r.Field(i1)
			if field_t.Tag.Get(`gorm`) == `-` || (gormGetID(uint_ptr, field_t) == 0) || fieldColumnMap[field_t.Name] == nil {
				continue
			}
			this.buffer.WriteString(fieldColumnMap[field_t.Name].ColumnName)
			this.buffer.WriteByte(61)
	...
	DEBUG("insert语句", this.buffer.String(), this.prepare_arg)
...
func (this *Mysql_Build) Replace(i interface{}) (err error) {
	...
	case reflect.Struct:
		fieldColumnMap := this.db.StructKeyColumn[r.Name()]
		if fieldColumnMap == nil {
			return errors.New("未初始化的struct 名称 " + r.Name())
		}
		for i1 := 0; i1 < r.NumField(); i1++ {
			field_t := r.Field(i1)
			if field_t.Tag.Get(`gorm`) == `-` || fieldColumnMap[field_t.Name] == nil {
				continue
			}
			this.buffer.WriteString(fieldColumnMap[field_t.Name].ColumnName)
			this.buffer.WriteByte(61)
	...
	//sql := do + this.sql.attr + ` INTO` + this.sql.table + ` SET ` + this.extend_data(param)
	DEBUG("Replace语句", this.buffer.String())
...
func (this *Mysql_Build) InsertAll(i interface{}) (res bool, err error) {
	...
	for i := 0; i < s.Len; i++ {
		...
		case reflect.Struct:
			fieldColumnMap := this.db.StructKeyColumn[r.Name()]
			if fieldColumnMap == nil {
				return false, errors.New("未初始化的struct 名称 " + r.Name())
			}
			vv := []string{}
			for i1 := 0; i1 < t.NumField(); i1++ {
				field_t := t.Field(i1)
				if gormGetID(s_uint_ptr, field_t) == 0 || field_t.Tag.Get(`gorm`) == `-` || fieldColumnMap[field_t.Name] == nil {
					continue
				}
				if i == 0 {
					//取出key的排列
					field = append(field, fieldColumnMap[field_t.Name].ColumnName)
				}
...
func (this *Mysql_Build) ReplaceAll(i interface{}) (res bool, err error) {
	...
	DEBUG("replace语句"+this.buffer.String(), this.prepare_arg)
	_, rowsAffected, e := insert(this.buffer.Bytes(), this.prepare_arg, this.db, this.Transaction)
	if e != nil {
...
func (this *Mysql_Build) Delete() (result bool, err error) {
	...
	if this.sql.where.Len() == 0 {
		return false, errors.New("Delete 删除不带where条件")
	}
	this.buffer.Reset()
	...
	DEBUG("delete语句"+this.buffer.String(), this.prepare_arg)
	if e != nil {
...
func (this *Mysql_Build) Update(param interface{}, arg ...interface{}) (result bool, err error) {
	...
	DEBUG("update语句"+this.buffer.String(), this.prepare_arg)
	if e != nil {
...
func (this *Mysql_Build) Find(s interface{}) (err error) {
	...
	e := query(this.buffer.Bytes(), this.prepare_arg, this.db, this.Transaction, s)
	DEBUG(`find的sql语句`, this.buffer.String(), this.prepare_arg, s, e)
	if e != nil {
...
func gormGetID(ptr uintptr, field reflect.StructField) int {
	if field.Type.String() == "gorm.Model" {
		return int(*((*uint)(unsafe.Pointer(ptr + field.Offset))))
	}
	return -1
}
